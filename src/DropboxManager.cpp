...
{
	DMSG_NL("Setting Dropbox access token");
	_token = token;
}
...
bool DropboxManager::stringUpload(String str, String address, bool type)
{
	...
	if (_token != '\0')
	{
		...
		{
			DMSG_NL("file Created");
			_http.end();
			return true;
		}
		DMSG_NL("error ;)");
		DMSG_NL("httpCode : %d", httpCode);
		response = _http.getString();
		DMSG_NL("%s", response.c_str());
		_http.end();
		return false;
	}
	DMSG_NL("Please Make Sure You have set valid token");
	return false;
}
...
bool DropboxManager::fileUpload(String file, String address, bool type)
{
	...
	if (_token != '\0')
	{
		if (!SPIFFS.begin())
		{
			DMSG_NL("Failed To Mount FS");
			return false;
		}
		File fileUpload = SPIFFS.open(file, "r");
		if (!fileUpload)
		{
			DMSG_NL("No Such file Found to Upload;)");
			return false;
		}
		uint32_t file_size = fileUpload.size();
		if (file_size > 0)
		{
			DMSG_NL("File %s size is : %lu", file.c_str(), file_size);
			DMSG_NL("%ld", file_size);
			_auth = "Bearer " + _token;
			String DROP_URL = String(DROPBOX_CONTENT_API) + String(DROPBOX_FILE_UPLOAD);
			StaticJsonDocument<512> Arg;
			Arg["path"] = address;
			...
			if (httpCode == HTTP_CODE_OK)
			{
				DMSG_NL("File uploaded/Created...");
				return true;
			}
			DMSG_NL("code : %d", httpCode);
			DMSG_NL("response : %s", response.c_str());
			return false;
		}
		DMSG_NL("File %s not found or valid ;)");
		fileUpload.close();
		SPIFFS.end();
	}
	return false;
...
bool DropboxManager::fileDownload(String file, String address)
{
	...
	if (_token != '\0')
	{
		...
		if (httpCode == HTTP_CODE_OK)
		{
			...
			DMSG_NL("Downloading File To Spiffs using stream...");
			File fileDownload = SPIFFS.open(file, "w");
			if (fileDownload)
			{
				_http.writeToStream(&fileDownload);
				DMSG_NL("File Downloaded:)");
			}
			else
			{
				DMSG_NL("can't Access Spiffs ");
			}
			fileDownload.close();
			_http.end();
			SPIFFS.end();
		...
		_http.end();
		DMSG_NL("code : %d", httpCode);
		DMSG_NL("Failed To Download File ;)");
		return false;
	}
}
